...
if($monster){
     ...
     * Cleanup mob.
     */
    public function cleanupMobs(){

        $maxTotal = (int) $this->getConfig()->getNested(
            "cleanup.max-total",
            82
        );

        $protected = $this->getConfig()->getNested(
            "cleanup.protected",
            array()
        );
        ...
        foreach($this->getServer()->getLevels() as $level){
            ...
            foreach($level->getEntities() as $entity){
                ...
                if(
                    $entity instanceof
                    \milk\pureentities\entity\animal\Animal
                    ||
                    $entity instanceof
                    \milk\pureentities\entity\monster\Monster
                ){
